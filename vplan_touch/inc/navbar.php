...
<nav id=thenav>
	...
	<a <?php if($school_config["home"]["tiles"]["forms"]["auto_select_page"]>0){echo "href=\"?nav=home&cat=forms&subnav=page".$school_config["home"]["tiles"]["forms"]["auto_select_page"]."\"";} else {echo 'onclick="showLinkgroup(\'forms\');"';}?> class="<?php if($_GET["cat"]=="forms")echo"active ";?>waves-effect waves-yellow"><span class="glyphicon glyphicon-user"></span>&nbsp;<?php echo t("Klassen");?></a>
	<?php
	if($teachers_enabled=="1") {
	?><a <?php if($school_config["home"]["tiles"]["teachers"]["auto_select_page"]>0){echo "href=\"?nav=home&cat=teachers&subnav=page".$school_config["home"]["tiles"]["teachers"]["auto_select_page"]."\"";} else {echo 'onclick="showLinkgroup(\'teachers\');"';}?> class="<?php if($_GET["cat"]=="teachers")echo"active ";?>waves-effect waves-yellow"><span class="glyphicon glyphicon-education"></span>&nbsp;<?php echo t("Lehrer");?></a>
	<a <?php if($school_config["home"]["tiles"]["rooms"]["auto_select_page"]>0){echo "href=\"?nav=home&cat=rooms&subnav=page".$school_config["home"]["tiles"]["rooms"]["auto_select_page"]."\"";} else {echo 'onclick="showLinkgroup(\'rooms\');"';}?> class="<?php if($_GET["cat"]=="rooms")echo"active ";?>waves-effect waves-yellow"><span class="glyphicon glyphicon-home"></span>&nbsp;<?php echo t("Räume");?></a>
	<?php
	}
	?>
